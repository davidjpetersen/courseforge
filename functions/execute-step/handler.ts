...
export interface ExecuteStepDeps {
  ...
  s3Client: { putObject(params: Record<string, unknown>): Promise<unknown> };
  cloudWatchClient: { putMetricData(params: Record<string, unknown>): Promise<unknown> };
...
export function createExecuteStepHandler(deps: ExecuteStepDeps) {
  ...
  return async (event: ExecuteStepEvent): Promise<Record<string, unknown>> => {
    ...
    const subsegment = {
      close: (..._args: unknown[]) => undefined,
      addError: (..._args: unknown[]) => undefined,
    };

    try {
      const result = await connector.run(event.step.params, {
        variables: Object.fromEntries(
          Object.entries(event.accumulatedContext ?? {}).map(([key, value]) => [key, String(value)]),
        ),
        workflowId: 'unknown',
      ...
      let inlineOutput: unknown = result;
      ...
      await deps.dynamoClient.send(
        new UpdateCommand({
          ...
          Key: { PK: `RUN#${event.runId}`, SK: stepKey },
          UpdateExpression:
            'SET #status = :status, endedAt = :endedAt, output = :output, outputRef = :outputRef',
          ExpressionAttributeNames: { '#status': 'status' },
      ...
      const normalizedError = error as { message?: string; code?: string; rawResponse?: unknown };
