      ...
      fc.asyncProperty(
        ...
        async (users) => {
          ...
          const subscribed = users.filter(
            (user) =>
              user.notificationPrefs?.workflowId === 'all' ||
              user.notificationPrefs?.workflowId === 'wf-1',
          ).length;

          const batchCalls = send.mock.calls
            .slice(1)
            .map((call) => call[0] as BatchWriteCommand);
          const written = batchCalls.reduce(
            (count, command) =>
              count + (command.input.RequestItems?.['courseforge-main']?.length ?? 0),
            0,
          );
