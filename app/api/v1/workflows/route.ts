...
export async function POST(request: NextRequest) {
  const mw = await runV1Middleware(request, 'POST', '/api/v1/workflows');
  if (mw.error) return mw.error;

  const body = await request.json();
  const result = await handler.create(mw.auth.tenantId, body);
  return new NextResponse(result.body, { status: result.statusCode, headers: result.headers });
}

export async function GET(request: NextRequest) {
  const mw = await runV1Middleware(request, 'GET', '/api/v1/workflows');
  if (mw.error) return mw.error;
