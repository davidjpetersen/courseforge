// ../functions/run-initializer/handler.ts
...
function workflowSK(workflowId) {
  return `${KEY_PREFIX.WORKFLOW}${workflowId}`;
}

// ../functions/shared/run-records.ts
async function findRunRecordById(dynamoClient2, tableName, tenantId, runId) {
  const result = await dynamoClient2.query({
    TableName: tableName,
...
function createRunInitializerHandler(deps) {
  ...
  return async (input) => {
    ...
    if (workflowMeta.Item.status !== "PUBLISHED" || typeof versionId !== "string" || versionId.length === 0) {
...
var handler = createRunInitializerHandler({
  dynamoClient: {
    async get(params) {
