...
const arbWorkflowStatus = fc.constantFrom<WorkflowStatus>('DRAFT', 'PUBLISHED', 'PAUSED', 'ARCHIVED');
const arbEnvironmentId = fc.constantFrom<'dev' | 'prod'>('dev', 'prod');
const arbId = fc.uuid();
...
const arbWorkflowRecord = fc.record<WorkflowRecord>({
  PK: fc.string(),
  SK: fc.string(),
  workflowId: arbId,
  tenantId: arbId,
...
const arbStepDefinition = fc.record<StepDefinition>({
...
const arbVersionRecord = (semver?: fc.Arbitrary<string>) =>
  fc.record<WorkflowVersionRecord>({
    PK: fc.string(),
    SK: fc.string(),
    versionId: arbId,
    workflowId: arbId,
...
const arbConnectionStatus = fc.constantFrom<Connection['status']>('active', 'error', 'pending', 'deleted');

const arbConnection = fc.record<Connection>({
  connectionId: arbId,
  tenantId: arbId,
...
describe('Feature: workflow-management-api, Property 3: Workflow List Filtering', () => {
  it('returns all workflows when no filters are provided', () => {
    fc.assert(
      fc.property(fc.array(arbWorkflowRecord, { maxLength: 20 }), (workflows) => {
  ...
  it('returns only workflows matching the status filter', () => {
    fc.assert(
      fc.property(
        fc.array(arbWorkflowRecord, { maxLength: 20 }),
        arbWorkflowStatus,
      ...
      fc.property(
        ...
        (workflows, envFilter) => {
          ...
          expect(result).toHaveLength(
            workflows.filter((w) => w.environmentId === envFilter).length,
          );
        },
      ),
      { numRuns: 100 },
...
describe('Feature: workflow-management-api, Property 5: Version Sorting by Semver Descending', () => {
  it('sorts versions so each adjacent pair satisfies compareSemver(a, b) >= 0', () => {
    fc.assert(
      fc.property(fc.array(arbVersionRecord(), { maxLength: 10 }), (versions) => {
...
describe('Feature: workflow-management-api, Property 17: Workflow Lifecycle State Machine', () => {
  ...
  it('returns true if and only if the pair is one of the five valid transitions', () => {
    fc.assert(
      fc.property(arbWorkflowStatus, arbWorkflowStatus, (from, to) => {
  ...
  it('always returns false for transitions from ARCHIVED', () => {
    fc.assert(
      fc.property(arbWorkflowStatus, (to) => {
...
describe('Feature: workflow-management-api, Property 1: Create Workflow Request Validation', () => {
  ...
  it('accepts bodies with all required fields valid', () => {
    fc.assert(
      fc.property(
        ...
        arbValidRecipeId,
        arbValidEnv,
        arbValidConnectionIds,
        (name, recipeId, environmentId, connectionIds) => {
  ...
  it('rejects bodies with missing or empty name', () => {
    ...
    fc.assert(
      fc.property(arbBadName, arbValidRecipeId, arbValidEnv, arbValidConnectionIds, (name, recipeId, environmentId, connectionIds) => {
  ...
  it('rejects bodies with missing or empty recipeId', () => {
    ...
    fc.assert(
      fc.property(arbValidName, arbBadRecipeId, arbValidEnv, arbValidConnectionIds, (name, recipeId, environmentId, connectionIds) => {
        expect(validateCreateRequest({ name, recipeId, environmentId, connectionIds })).not.toBeNull();
      }),
      { numRuns: 100 },
    );
  ...
  it('rejects bodies with invalid environmentId', () => {
    ...
    fc.assert(
      fc.property(arbValidName, arbValidRecipeId, arbBadEnv, arbValidConnectionIds, (name, recipeId, environmentId, connectionIds) => {
        expect(validateCreateRequest({ name, recipeId, environmentId, connectionIds })).not.toBeNull();
      }),
      { numRuns: 100 },
    );
  ...
  it('rejects bodies with non-array connectionIds', () => {
    const arbBadConnectionIds = fc.oneof(
      fc.constant(null),
      fc.constant(undefined),
      fc.string(),
      fc.integer(),
    ...
    fc.assert(
      fc.property(arbValidName, arbValidRecipeId, arbValidEnv, arbBadConnectionIds, (name, recipeId, environmentId, connectionIds) => {
        expect(validateCreateRequest({ name, recipeId, environmentId, connectionIds })).not.toBeNull();
      }),
      { numRuns: 100 },
    );
...
describe('Feature: workflow-management-api, Property 2: Connection Status Validation', () => {
  it('accepts when every requested connectionId exists and is active', () => {
    fc.assert(
      fc.property(
        ...
        (ids) => {
          const connections: Connection[] = ids.map((id) => ({
            connectionId: id,
            tenantId: 'tenant-1',
            status: 'active',
  ...
  it('rejects when a requested connectionId is missing', () => {
    fc.assert(
      fc.property(
        fc.array(arbId, { maxLength: 5 }),
        arbId,
        (presentIds, missingId) => {
          fc.pre(!presentIds.includes(missingId));
          const connections: Connection[] = presentIds.map((id) => ({
            connectionId: id,
            tenantId: 'tenant-1',
  ...
  it('rejects when a requested connection has non-active status', () => {
    const arbInactiveStatus = fc.constantFrom<Connection['status']>('error', 'pending', 'deleted');
    fc.assert(
      fc.property(arbId, arbInactiveStatus, (id, status) => {
        const connections: Connection[] = [{ connectionId: id, tenantId: 'tenant-1', status, secretRef: 'arn' }];
  ...
  it('accepts empty request set with any connections', () => {
    fc.assert(
      fc.property(fc.array(arbConnection, { maxLength: 5 }), (connections) => {
