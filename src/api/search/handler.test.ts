...
function makeClient(templates: Template[]): SearchClient {
  return {
    search: async (query, categories) =>
      searchTemplates(templates, query, categories),
...
describe('GET /search handler', () => {
  it('returns 200 with results for a matching query', async () => {
    ...
    const result = await handler(
      makeEvent({ queryStringParameters: { q: 'roster' } }),
    );
  ...
  it('returns zero results with suggestions for non-matching query', async () => {
    const handler = createSearchHandler(makeClient(SEED_TEMPLATES));
    const result = await handler(
      makeEvent({ queryStringParameters: { q: 'zzzznothing' } }),
    );
  ...
  it('returns 503 when search client throws', async () => {
    const failingClient: SearchClient = {
  ...
  it('handles special characters in query', async () => {
    const handler = createSearchHandler(makeClient(SEED_TEMPLATES));
    const result = await handler(
      makeEvent({ queryStringParameters: { q: '<script>alert("xss")</script>' } }),
    );
