...
function getBaseUrl(credentials: Record<string, unknown>, fallback?: string): string | null {
  const candidate =
    fallback ??
...
function getRequiredString(secret: SecretValue, key: string): string | null {
  const value = secret[key];
  return typeof value === 'string' && value.trim() !== '' ? value : null;
}

const oauthStub: ConnectorDefinition['testFn'] = async () => ({
  success: false,
