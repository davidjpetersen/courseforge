...
function makeResponse(overrides: Partial<PublishResponse> = {}): PublishResponse {
  return {
    workflowId: 'wf-001',
    status: 'active',
    name: 'My Workflow',
...
describe('buildPublishConfirmationViewModel', () => {
  ...
  it('preserves workflow-template-tenant association', () => {
    const vm = buildPublishConfirmationViewModel(
      makeResponse({ workflowId: 'wf-xyz' }),
      makeRequest({ templateId: 'tpl-notifications', tenantId: 'tenant-school' }),
    );
