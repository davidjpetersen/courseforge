...
describe('Filter state persistence', () => {
  it('round-trips filter state through storage', () => {
    ...
    expect(getSelectedCategories(restored).sort()).toEqual(
      getSelectedCategories(state).sort(),
    );
  });
  ...
  it('filters out invalid category names from storage', () => {
    ...
    storage.setItem(
      ...
      JSON.stringify(['Roster Ops', 'InvalidCategory', 'Analytics']),
    );
    const restored = loadFilterState(storage);
