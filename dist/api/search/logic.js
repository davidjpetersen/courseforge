...
export function toSearchResult(template) {
    return {
 ...
 */
export function searchTemplates(templates, query, categories) {
 ...
 */
export function generateSuggestions(query, allTemplates, selectedCategories) {
    const suggestions = [];
    const normalizedQuery = query.trim().toLowerCase();
    ...
    if (normalizedQuery !== '') {
        ...
        if (words.length > 1) {
            for (const word of words) {
                ...
                const wouldMatch = allTemplates.some((t) => t.name.toLowerCase().includes(word) ||
 ...
 */
export function buildSearchResponse(allTemplates, query, categories) {
