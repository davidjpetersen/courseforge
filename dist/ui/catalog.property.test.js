...
function arbTemplate() {
    return fc
        .tuple(fc.uuid(), fc.string({ minLength: 1, maxLength: 40 }), fc.string({ minLength: 0, maxLength: 100 }), arbCategories, arbConnectionSet, fc.string({ minLength: 1, maxLength: 20 }), arbTagSet, fc.array(fc.record({
        fieldId: fc.uuid(),
        label: fc.string({ minLength: 1, maxLength: 30 }),
        type: fc.constantFrom('text', 'select', 'number', 'boolean', 'connection'),
        required: fc.boolean(),
        helpText: fc.string({ minLength: 0, maxLength: 50 }),
        validation: fc.constant({}),
        connectedSystem: fc.constant(null),
    }), { minLength: 0, maxLength: 4 }), fc.array(fc.record({
        stepIndex: fc.nat({ max: 10 }),
        title: fc.string({ minLength: 1, maxLength: 30 }),
        helpText: fc.string({ minLength: 0, maxLength: 50 }),
        fields: fc.constant([]),
    }), { minLength: 0, maxLength: 5 }))
        .map(([id, name, description, categories, connectedSystems, timeToActivate, tags, requiredParameters, steps,]) => ({
        templateId: id,
        name,
        description,
        categories,
...
const arbTemplateList = fc.array(arbTemplate(), {
    minLength: 0,
    maxLength: 10,
});
...
describe('Feature: recipe-library, Property 2: Template Grouping by Category', () => {
    ...
    it('places each template under every category it belongs to and no others', () => {
        fc.assert(fc.property(arbTemplateList, (templates) => {
            ...
            const totalGroupEntries = Object.values(grouped).reduce((sum, arr) => sum + arr.length, 0);
            const expectedTotal = catalog.templates.reduce((sum, s) => sum + s.categories.length, 0);
...
describe('Feature: recipe-library, Property 3: Template View Renders Required Fields', () => {
    ...
    it('detail view contains all required detail fields', () => {
        fc.assert(fc.property(arbTemplate(), arbConnectionSet, (template, tenantConnections) => {
...
//# sourceMappingURL=catalog.property.test.js.map