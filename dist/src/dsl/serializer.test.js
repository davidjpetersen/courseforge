...
describe('deserializeConfig — malformed DSL', () => {
    it('throws on null input', () => {
        expect(() => deserializeConfig(null)).toThrow(DSLValidationError);
    });
    it('throws on undefined input', () => {
        expect(() => deserializeConfig(undefined)).toThrow(DSLValidationError);
    });
    it('throws on non-object input', () => {
        expect(() => deserializeConfig('bad')).toThrow(DSLValidationError);
    });
    it('throws with section "version" when version is missing', () => {
        const dsl = { templateId: 'x', name: 'n', steps: [], metadata };
        try {
            ...
            expect(e.section).toBe('version');
        }
    });
    it('throws with section "templateId" when templateId is missing', () => {
        const dsl = { version: '1.0', name: 'n', steps: [], metadata };
        try {
            deserializeConfig(dsl);
            expect.fail('should have thrown');
        }
            ...
            expect(e).toBeInstanceOf(DSLValidationError);
            expect(e.section).toBe('templateId');
        }
    });
    it('throws with section "name" when name is empty', () => {
        const dsl = { version: '1.0', templateId: 'x', name: '', steps: [], metadata };
        try {
            deserializeConfig(dsl);
            expect.fail('should have thrown');
        }
            ...
            expect(e).toBeInstanceOf(DSLValidationError);
            expect(e.section).toBe('name');
        }
    });
    it('throws with section "steps" when steps is not an array', () => {
        const dsl = { version: '1.0', templateId: 'x', name: 'n', steps: 'bad', metadata };
        try {
            deserializeConfig(dsl);
            expect.fail('should have thrown');
        }
            ...
            expect(e).toBeInstanceOf(DSLValidationError);
            expect(e.section).toBe('steps');
        }
    });
    it('throws with section "metadata" when metadata is missing', () => {
        const dsl = { version: '1.0', templateId: 'x', name: 'n', steps: [] };
        try {
            deserializeConfig(dsl);
            expect.fail('should have thrown');
        }
            ...
            expect(e).toBeInstanceOf(DSLValidationError);
            expect(e.section).toBe('metadata');
        }
    });
    it('throws with section "metadata" when metadata is an array', () => {
        const dsl = { version: '1.0', templateId: 'x', name: 'n', steps: [], metadata: [] };
        try {
            deserializeConfig(dsl);
            expect.fail('should have thrown');
        }
    ...
    it('throws when a step is missing stepIndex', () => {
        ...
        try {
            ...
            expect(e).toBeInstanceOf(DSLValidationError);
            expect(e.section).toBe('steps[0].stepIndex');
        }
    });
    it('throws when a step has parameters as an array', () => {
        ...
        try {
            ...
            expect(e).toBeInstanceOf(DSLValidationError);
            expect(e.section).toBe('steps[0].parameters');
        }
    });
    it('throws when metadata.tenantId is missing', () => {
        ...
        try {
            ...
            expect(e).toBeInstanceOf(DSLValidationError);
            expect(e.section).toBe('metadata.tenantId');
        }
    });
