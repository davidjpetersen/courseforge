...
const arbFieldValue = fc.oneof(fc.string({ minLength: 0, maxLength: 50 }), fc.double({ min: -1e6, max: 1e6, noNaN: true, noDefaultInfinity: true }), fc.integer({ min: -1000, max: 1000 }), fc.boolean(), fc.constant(null));
/** Generates a non-empty record of field values. */
const arbFields = fc.dictionary(fc.string({ minLength: 1, maxLength: 20 }).filter((s) => s.trim().length > 0), arbFieldValue, { minKeys: 1, maxKeys: 8 });
/** Generates a valid WizardStepConfig. */
function arbWizardStep(index) {
...
const arbWizardConfiguration = fc
    .integer({ min: 1, max: 5 })
    .chain((stepCount) => {
    const stepArbs = Array.from({ length: stepCount }, (_, i) => arbWizardStep(i));
    return fc.tuple(fc.string({ minLength: 1, maxLength: 40 }).filter((s) => s.trim().length > 0), fc.tuple(...stepArbs));
})
...
describe('Feature: recipe-library, Property 1: DSL Serialization Round-Trip', () => {
    ...
    it('round-trips any valid WizardConfiguration through serialize → deserialize', () => {
        fc.assert(fc.property(arbWizardConfiguration, arbNonEmptyString, arbMetadata, (config, name, metadata) => {
...
const arbMalformedDSL = fc.oneof(
// null / undefined / primitive
fc.constant(null), fc.constant(undefined), fc.string(), fc.integer(), fc.boolean(), 
// missing version
fc.record({
...
describe('Feature: recipe-library, Property 15: Malformed DSL Produces Descriptive Error', () => {
    ...
    it('throws DSLValidationError with non-empty message and section for any malformed DSL', () => {
        fc.assert(fc.property(arbMalformedDSL, (malformed) => {
            try {
                deserializeConfig(malformed);
...
//# sourceMappingURL=serializer.property.test.js.map