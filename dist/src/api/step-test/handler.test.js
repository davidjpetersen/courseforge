...
import { createStepTestHandler, validateStepTestRequest, } from './handler.js';
// ── Helpers ──
function makeProvider(systemMap = {}) {
    return {
        getConnectedSystemForStep: async (templateId, stepIndex) => systemMap[`${templateId}:${stepIndex}`] ?? null,
    };
}
function makeClient(response, error) {
    return {
        dryRun: async () => {
            if (error)
                throw error;
...
describe('validateStepTestRequest', () => {
    ...
    it('rejects non-object body', () => {
        expect(validateStepTestRequest(null)).toBe('Request body must be a JSON object');
        expect(validateStepTestRequest('string')).toBe('Request body must be a JSON object');
    });
    ...
    it('rejects negative stepIndex', () => {
        ...
        expect(result).toBe('stepIndex is required and must be a non-negative integer');
    });
...
describe('POST /steps/{stepId}/test handler', () => {
    it('returns pass result for valid dry-run', async () => {
        const handler = createStepTestHandler(makeProvider({ 'tpl-1:0': 'Canvas LMS' }), makeClient({
            result: 'pass',
    ...
    it('returns fail result with suggested fix', async () => {
        const handler = createStepTestHandler(makeProvider({ 'tpl-1:1': 'PowerSchool SIS' }), makeClient({
            result: 'fail',
        ...
        expect(body.suggestedFix).toBe('Check your API key in the connection settings');
    });
    it('returns 400 when body is missing', async () => {
        const handler = createStepTestHandler(makeProvider(), makeClient());
    ...
    it('returns 400 when step has no connected system', async () => {
        const handler = createStepTestHandler(makeProvider(), // no systems mapped
        makeClient());
        const result = await handler(makeEvent({
            body: JSON.stringify({
                templateId: 'tpl-1',
        ...
        expect(result.statusCode).toBe(400);
        expect(JSON.parse(result.body).message).toContain('does not reference a connected system');
    });
    it('returns 500 when connected system client throws (timeout simulation)', async () => {
        const handler = createStepTestHandler(makeProvider({ 'tpl-1:0': 'Canvas LMS' }), makeClient(undefined, new Error('Connection timed out')));
        const result = await handler(makeEvent({
            body: JSON.stringify({
                templateId: 'tpl-1',
                stepIndex: 0,
