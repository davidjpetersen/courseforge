...
const arbSearchQuery = fc.oneof(fc.constant(''), fc.stringOf(fc.constantFrom(...'abcdefghijklmnopqrstuvwxyz '.split('')), {
    minLength: 1,
    maxLength: 30,
}));
...
function arbTemplate() {
    return fc
        .tuple(fc.uuid(), fc.stringOf(fc.constantFrom(...'abcdefghijklmnopqrstuvwxyz '.split('')), {
        minLength: 1,
        maxLength: 40,
...
describe('Feature: recipe-library, Property 14: Combined Search and Filter Returns Intersection', () => {
    ...
    it('returns the intersection of query-matched and category-matched templates', () => {
        fc.assert(fc.property(arbTemplateList, arbSearchQuery, arbCategoryFilter, (templates, query, categories) => {
            const result = searchTemplates(templates, query, categories);
            const normalizedQuery = query.trim().toLowerCase();
            const categorySet = new Set(categories);
            // Compute expected sets independently
            const matchesQuery = (t) => normalizedQuery === '' ||
                t.name.toLowerCase().includes(normalizedQuery) ||
                t.description.toLowerCase().includes(normalizedQuery);
            const matchesCategory = (t) => categorySet.size === 0 ||
                t.categories.some((c) => categorySet.has(c));
            // Expected: intersection of both predicates
            const expected = templates.filter((t) => matchesQuery(t) && matchesCategory(t));
            // Same length
