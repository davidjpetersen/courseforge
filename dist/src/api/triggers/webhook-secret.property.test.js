...
describe('Property 9: Webhook secret hash round-trip', () => {
    it('SHA-256 hash of returned token equals stored tokenHash, and raw token is never stored', async () => {
        await fc.assert(fc.asyncProperty(fc.record({ workflowId: arbId, tenantId: arbId }), async ({ workflowId, tenantId }) => {
            let capturedItem;
            const put = vi.fn(async (params) => {
...
//# sourceMappingURL=webhook-secret.property.test.js.map