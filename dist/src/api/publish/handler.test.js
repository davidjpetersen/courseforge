...
import { createPublishHandler, validatePublishRequest, buildFirstRunUrl, } from './handler.js';
// ── Helpers ──
...
function makeSfnClient(workflow, error) {
    return {
...
describe('POST /workflows handler', () => {
    ...
    it('returns 500 with error message on publish failure (retry scenario)', async () => {
        const handler = createPublishHandler(makeSfnClient(undefined, new Error('Step Functions execution failed')));
        const result = await handler(makeEvent({ body: VALID_BODY }));
