...
import { createConnectionHandler, deleteConnectionHandler, getDependenciesHandler, listConnectionsHandler, rotateConnectionHandler, testConnectionHandler, } from './handlers.js';
import { connectorRegistry } from './registry.js';
function mockFetch(ok, status) {
...
function makeConnectionRepo(connection = makeConnection()) {
    return {
...
function makeWorkflowRepo(workflows = []) {
    return {
...
describe('connection handlers', () => {
    ...
    it('create returns 400 on bad input', async () => {
        const handler = createConnectionHandler(makeConnectionRepo(), makeSecretsService(), connectorRegistry);
        const result = await handler(makeEvent({
            httpMethod: 'POST',
            body: JSON.stringify({
                tenantId: 'tenant-1',
    ...
    it('test-connection returns 404 for missing connection', async () => {
        const handler = testConnectionHandler(makeConnectionRepo(null), makeSecretsService(), connectorRegistry);
        const result = await handler(makeEvent({
            httpMethod: 'POST',
            pathParameters: { id: 'conn-1' },
            queryStringParameters: { tenantId: 'tenant-1' },
    ...
    it('list returns mapped items', async () => {
        ...
        const result = await handler(makeEvent({ queryStringParameters: { tenantId: 'tenant-1' } }));
        expect(result.statusCode).toBe(200);
        expect(JSON.parse(result.body).connections).toHaveLength(1);
    });
    it('dependencies returns dependent workflows', async () => {
        const handler = getDependenciesHandler(makeConnectionRepo(), makeWorkflowRepo([
            {
                workflowId: 'wf-1',
                tenantId: 'tenant-1',
                templateId: 'tpl-1',
    ...
    it('rotate returns 200 on success', async () => {
        ...
        const handler = rotateConnectionHandler(makeConnectionRepo(), makeSecretsService(), connectorRegistry, audit);
        globalThis.fetch = mockFetch(true, 200);
        const result = await handler(makeEvent({
            httpMethod: 'PUT',
            pathParameters: { id: 'conn-1' },
    ...
    it('rotate returns 422 when credentials fail connector test', async () => {
        ...
        const handler = rotateConnectionHandler(makeConnectionRepo(), makeSecretsService(), connectorRegistry, makeAuditRepo());
        const result = await handler(makeEvent({
            httpMethod: 'PUT',
            pathParameters: { id: 'conn-1' },
            body: JSON.stringify({
    ...
    it('delete returns 204 on success', async () => {
        const handler = deleteConnectionHandler(makeConnectionRepo(), makeSecretsService(), makeWorkflowRepo([]), makeAuditRepo());
        const result = await handler(makeEvent({
            httpMethod: 'DELETE',
            pathParameters: { id: 'conn-1' },
            queryStringParameters: { tenantId: 'tenant-1' },
    ...
    it('delete returns 409 on published dependents', async () => {
        const handler = deleteConnectionHandler(makeConnectionRepo(), makeSecretsService(), makeWorkflowRepo([
            {
                workflowId: 'wf-1',
                tenantId: 'tenant-1',
                templateId: 'tpl-1',
