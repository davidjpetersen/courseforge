...
import { buildNewConnectionRecord, buildSecretName, filterDependentWorkflows, hasPublishedDependents, mapConnectionToListItem, mapTestResultToStatus, validateCredentials, } from './logic.js';
...
describe('Feature: connection-management properties', () => {
    it('property 1: credential validation accepts valid generic-http credentials', () => {
        fc.assert(fc.property(fc.webUrl(), (baseUrl) => {
            expect(validateCredentials('generic-http', { baseUrl }, connectorRegistry)).toEqual({ valid: true });
        }));
    });
    it('property 2: secret naming convention preserves tenant and connection ids', () => {
        fc.assert(fc.property(arbId, arbId, (tenantId, connectionId) => {
            expect(buildSecretName(tenantId, connectionId)).toBe(`courseforge/tenant/${tenantId}/connection/${connectionId}`);
        }));
    });
    it('property 3: new connection records always start pending with aligned timestamps', () => {
        fc.assert(fc.property(arbId, arbIso, (connectionId, now) => {
    ...
    it('property 5: test results map deterministically to status', () => {
        fc.assert(fc.property(fc.boolean(), (success) => {
            expect(mapTestResultToStatus({ success, message: 'x' })).toBe(success ? 'active' : 'error');
        }));
    });
    it('property 6: dependency filtering only returns workflows referencing the target id', () => {
        fc.assert(fc.property(fc.array(arbId, { maxLength: 5 }), arbId, (ids, targetId) => {
            const workflows = ids.map((id, index) => ({
                workflowId: `wf-${index}`,
                tenantId: 'tenant-1',
                templateId: 'tpl-1',
                name: `Workflow ${index}`,
            ...
            expect(dependents.every((dependent) => Number(dependent.workflowId.split('-')[1]) % 2 === 0)).toBe(true);
        }));
    });
    it('property 8: published dependency guard matches status semantics', () => {
        fc.assert(fc.property(fc.boolean(), (shouldBlock) => {
            expect(hasPublishedDependents([
                {
                    workflowId: 'wf-1',
                    name: 'A',
