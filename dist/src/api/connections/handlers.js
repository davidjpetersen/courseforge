...
function isApiGatewayResult(value) {
...
function validateRotateRequest(body) {
    if (!isObject(body)) {
        return 'Request body must be a JSON object';
    }
    if (typeof body.tenantId !== 'string' || body.tenantId.trim() === '') {
...
export function createConnectionHandler(repo, secrets, registry) {
    return async (event) => {
        try {
            ...
            const validation = validateCredentials(request.connectorKey, request.credentials, registry);
            if (!validation.valid) {
            ...
            const secretName = buildSecretName(request.tenantId, provisionalRecord.connectionId);
...
export function testConnectionHandler(repo, secrets, registry) {
    return async (event) => {
        try {
...
export function listConnectionsHandler(repo) {
    return async (event) => {
        try {
            const tenantId = event.queryStringParameters?.tenantId;
            if (!tenantId) {
...
export function getDependenciesHandler(repo, workflowRepo) {
    return async (event) => {
        try {
            const connectionId = event.pathParameters?.id;
            const tenantId = event.queryStringParameters?.tenantId;
...
export function rotateConnectionHandler(repo, secrets, registry, audit) {
    return async (event) => {
        try {
            const connectionId = event.pathParameters?.id;
            if (!connectionId) {
            ...
            const validation = validateCredentials(connection.connectorKey, request.credentials, registry);
            if (!validation.valid) {
                return jsonResponse(400, { errors: validation.errors });
            }
            const connector = registry.get(connection.connectorKey);
            ...
            const testResult = await connector.testFn(request.credentials, String(request.credentials.baseUrl ?? ''));
            ...
            await audit.writeEntry(buildAuditEntry(request.tenantId, 'CONNECTION_ROTATED', getHeader(event.headers, 'x-actor') ?? 'system', connectionId, getHeader(event.headers, 'x-forwarded-for') ?? 'unknown'));
...
export function deleteConnectionHandler(repo, secrets, workflowRepo, audit) {
    return async (event) => {
        try {
            const connectionId = event.pathParameters?.id;
            const tenantId = event.queryStringParameters?.tenantId;
            ...
            await audit.writeEntry(buildAuditEntry(tenantId, 'CONNECTION_DELETED', getHeader(event.headers, 'x-actor') ?? 'system', connectionId, getHeader(event.headers, 'x-forwarded-for') ?? 'unknown'));
            return {
                statusCode: 204,
