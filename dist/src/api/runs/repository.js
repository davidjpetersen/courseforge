...
import { decodeCursor } from './validation.js';
function toRun(item) {
    return {
        ...
        triggerType: String(item.triggerType ?? 'webhook'),
...
function isWithinDateRange(run, params) {
...
async function queryRuns(client, tableName, options) {
...
export function createDynamoRunRepository(client, tableName) {
    return {
        ...
        async getSteps(runId) {
            ...
            return (result.Items ?? []).map((item) => toRunStep(item));
        },
    };
}
