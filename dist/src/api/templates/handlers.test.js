...
import { createListTemplatesHandler, createGetTemplateHandler, } from './handlers.js';
import { ROSTER_OPS_TEMPLATE, NOTIFICATIONS_TEMPLATE, } from '../../data/seed-templates.js';
// ── Helpers ──
...
function makeConnectionProvider(connections = []) {
    return {
...
describe('GET /templates handler', () => {
    it('returns all templates when no category filter is provided', async () => {
        const handler = createListTemplatesHandler(makeRepo([ROSTER_OPS_TEMPLATE, NOTIFICATIONS_TEMPLATE]));
...
describe('GET /templates/{templateId} handler', () => {
    it('returns template detail with stepCount and missingConnections', async () => {
        const handler = createGetTemplateHandler(makeRepo([ROSTER_OPS_TEMPLATE]), makeConnectionProvider(['Canvas LMS']));
        const result = await handler(makeEvent({
    ...
    it('returns 404 when template does not exist', async () => {
        const handler = createGetTemplateHandler(makeRepo([]), makeConnectionProvider());
        const result = await handler(makeEvent({ pathParameters: { templateId: 'nonexistent' } }));
        expect(result.statusCode).toBe(404);
        const body = JSON.parse(result.body);
        expect(body.message).toBe('Template not found');
    });
    it('returns 400 when templateId path parameter is missing', async () => {
        const handler = createGetTemplateHandler(makeRepo([ROSTER_OPS_TEMPLATE]), makeConnectionProvider());
    ...
    it('returns no missing connections when all are configured', async () => {
        const handler = createGetTemplateHandler(makeRepo([ROSTER_OPS_TEMPLATE]), makeConnectionProvider(['PowerSchool SIS', 'Canvas LMS']));
        const result = await handler(makeEvent({
            pathParameters: { templateId: ROSTER_OPS_TEMPLATE.templateId },
        }));
        expect(result.statusCode).toBe(200);
    ...
    it('returns 500 when repository throws', async () => {
        ...
        const result = await handler(makeEvent({ pathParameters: { templateId: 'any' } }));
        expect(result.statusCode).toBe(500);
    });
});
