...
import { filterByCategory, detectMissingConnections } from './logic.js';
// ── Generators ──
...
function arbTemplate() {
    return fc.tuple(fc.uuid(), fc.string({ minLength: 1, maxLength: 40 }), fc.string({ minLength: 0, maxLength: 100 }), arbCategories, fc.array(fc.string({ minLength: 1, maxLength: 30 }), { minLength: 0, maxLength: 4 })).map(([id, name, description, categories, connectedSystems]) => ({
        templateId: id,
        name,
        description,
        categories,
...
describe('Feature: recipe-library, Property 12: Category Filtering Returns Correct Subset', () => {
    ...
    it('returns exactly the correct subset for any templates and category filter', () => {
        fc.assert(fc.property(arbTemplateList, arbCategoryFilter, (templates, selectedCategories) => {
            ...
            if (selectedCategories.length === 0) {
                ...
                for (const t of result) {
                    expect(t.categories.some((c) => categorySet.has(c))).toBe(true);
                }
...
describe('Feature: recipe-library, Property 4: Missing Connection Detection', () => {
    ...
    it('returns the exact set difference for any template systems and tenant connections', () => {
        fc.assert(fc.property(arbConnectionSet, arbConnectionSet, (templateSystems, tenantConnections) => {
            const missing = detectMissingConnections(templateSystems, tenantConnections);
            const tenantSet = new Set(tenantConnections);
            const expectedMissing = templateSystems.filter((sys) => !tenantSet.has(sys));
