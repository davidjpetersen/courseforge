...
import { templatePK, templateSK, categoryGSI1PK, categoryGSI1SK, tenantPK, workflowSK, workflowPK, workflowMetaSK, connectionSK, auditSK, webhookSecretSK, runSK, schedulePK, scheduleSK, buildSecretName, buildCategoryIndexKeys, TABLE_DEFINITION, CATEGORY_INDEX, runPK, stepSK, userPK, notificationSK, GSI_WORKFLOW_RUNS, GSI_TENANT_STATUS, } from './schema.js';
describe('DynamoDB key builders', () => {
    ...
    it('builds run SK', () => {
        expect(runSK('2024-01-01T00:00:00Z', 'run-1')).toBe('RUN#2024-01-01T00:00:00Z#run-1');
    });
...
describe('run-history-observability key builders', () => {
    ...
    it('builds notification SK', () => {
        expect(notificationSK('2024-01-01T00:00:00Z', 'notif-1')).toBe('NOTIFICATION#2024-01-01T00:00:00Z#notif-1');
    });
