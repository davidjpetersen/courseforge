...
import { validateField, validateStep, validateAllSteps, findFirstErrorStep, buildValidationSummary, isPublishEnabled, } from './validation.js';
// ── Helpers ──
...
function makeStep(stepIndex, fields) {
    return { stepIndex, title: `Step ${stepIndex}`, helpText: '', fields };
}
function makeStepState(stepIndex, fields) {
...
describe('validateField — type validation', () => {
    ...
    it('rejects string value for number field', () => {
        expect(validateField(numField({ required: true }), 'abc')).toBe('Expected a numeric value');
    });
...
describe('validateField — pattern matching', () => {
    it('rejects value that does not match pattern', () => {
        ...
        expect(validateField(f, 'abc')).toBe('Name does not match the required format');
    });
...
describe('validateField — min/max for strings (length)', () => {
    ...
    it('rejects string longer than max', () => {
        ...
        expect(validateField(f, 'abcdef')).toBe('Name must be at most 5 characters');
    });
...
describe('validateAllSteps', () => {
    ...
    it('returns empty when all steps valid', () => {
        ...
        const state = [makeStepState(0, { a: 'ok' })];
