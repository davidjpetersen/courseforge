...
import { createWizardState, getProgressIndicator, getCurrentStep, goToStep, setFieldValue, } from './wizard.js';
// ── Generators ──
const arbFieldType = fc.constantFrom('text', 'select', 'number', 'boolean', 'connection');
function arbFieldDefinition(index) {
    return fc
        .tuple(fc.string({ minLength: 1, maxLength: 20 }), arbFieldType, fc.boolean(), fc.string({ minLength: 1, maxLength: 40 }), fc.option(fc.string({ minLength: 1, maxLength: 20 }), { nil: null }))
...
function arbStepDefinition(stepIndex) {
    return fc
        .tuple(fc.string({ minLength: 1, maxLength: 30 }), fc.string({ minLength: 1, maxLength: 50 }), fc.integer({ min: 1, max: 5 }))
        .chain(([title, helpText, fieldCount]) => {
        const fieldArbs = Array.from({ length: fieldCount }, (_, i) => arbFieldDefinition(stepIndex * 10 + i));
...
const arbFieldValue = fc.oneof(fc.string({ minLength: 0, maxLength: 30 }), fc.integer({ min: -1000, max: 1000 }), fc.boolean());
...
describe('Feature: recipe-library, Property 6: Progress Indicator Accuracy', () => {
    ...
    it('displays correct current step and total for any step position', () => {
        fc.assert(fc.property(arbTemplate().chain((template) => fc.tuple(fc.constant(template), fc.integer({ min: 0, max: template.steps.length - 1 }))), ([template, stepIndex]) => {
...
describe('Feature: recipe-library, Property 7: Backward Navigation Preserves Data', () => {
    ...
    it('preserves all field values after backward and forward navigation', () => {
        fc.assert(fc.property(arbTemplate().chain((template) => {
            // Generate a value for every field in every step
            const fieldValueArbs = template.steps.map((step) => fc.tuple(...step.fields.map(() => arbFieldValue)));
            return fc.tuple(fc.constant(template), fc.tuple(...fieldValueArbs));
...
//# sourceMappingURL=wizard.property.test.js.map