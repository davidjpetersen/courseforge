...
const arbPublishRequest = fc.record({
    templateId: arbNonEmptyString,
    tenantId: arbNonEmptyString,
    name: arbNonEmptyString,
    configuration: fc.dictionary(fc.string({ minLength: 1, maxLength: 20 }), fc.oneof(fc.string(), fc.integer(), fc.boolean())),
});
const arbPublishResponse = fc.record({
...
describe('Feature: recipe-library, Property 11: Publish Confirmation Contains Required Fields', () => {
    it('confirmation contains workflow name, status, monitoring link, template ID, and tenant ID', () => {
        fc.assert(fc.property(arbPublishResponse, arbPublishRequest, (response, request) => {
...
//# sourceMappingURL=publish.property.test.js.map