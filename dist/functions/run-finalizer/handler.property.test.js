...
const arbStatus = fc.constantFrom('SUCCESS', 'FAILED');
...
describe('RunFinalizer properties', () => {
    it('publishes a domain event that round-trips through JSON serialization', async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1 }), fc.string({ minLength: 1 }), fc.string({ minLength: 1 }), arbStatus, arbJsonSafe, async (tenantId, workflowId, runId, status, extraPayload) => {
            ...
            const payload = (putEvents.mock.calls[0]?.[0]).Entries[0]?.Detail;
