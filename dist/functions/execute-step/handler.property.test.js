...
describe('ExecuteStep handler properties', () => {
    it('resolves known connector keys and rejects unknown ones', async () => {
        await fc.assert(fc.asyncProperty(fc.string(), async (connectorKey) => {
    ...
    it('offloads outputs iff their serialized size exceeds 4KB', async () => {
        await fc.assert(fc.asyncProperty(arbJsonSafe, async (output) => {
    ...
    it('preserves existing context keys when adding the current step result', async () => {
        await fc.assert(fc.asyncProperty(fc.dictionary(fc.string({ minLength: 1 }), arbJsonSafe), arbJsonSafe, async (context, stepResult) => {
            const handler = createExecuteStepHandler({
                dynamoClient: {
                    put: vi.fn(async () => ({})),
                    update: vi.fn(async () => ({})),
