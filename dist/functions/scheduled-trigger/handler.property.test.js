...
import { schedulePK, scheduleSK, tenantPK, workflowMetaSK, workflowPK, } from '../../src/models/schema.js';
...
const arbId = fc.stringOf(fc.mapToConstant({ num: 26, build: (n) => String.fromCharCode(97 + n) }), { minLength: 1, maxLength: 20 });
 ...
 */
function buildPublishedDeps(tenantId, workflowId) {
...
describe('Feature: event-triggering-routing, Property 4: Non-PUBLISHED workflow silent return', () => {
    ...
    it('returns without side-effects for non-PUBLISHED or missing workflows', async () => {
        await fc.assert(fc.asyncProperty(arbId, arbId, arbId, fc.oneof(fc.constantFrom('DRAFT', 'PAUSED', 'ARCHIVED'), fc.constant(null)), async (tenantId, workflowId, scheduleId, statusOrMissing) => {
            const putEvents = vi.fn(async () => ({}));
            const dynamoPut = vi.fn(async () => ({}));
            const dynamoUpdate = vi.fn(async () => ({}));
            const warnSpy = vi.fn();
    ...
    it('also returns without side-effects when tenantId mismatches', async () => {
        await fc.assert(fc.asyncProperty(arbId, arbId, arbId, arbId, async (tenantId, differentTenantId, workflowId, scheduleId) => {
...
describe('Feature: event-triggering-routing, Property 5: Successful scheduled trigger produces complete outputs', () => {
    ...
    it('publishes event, writes run record with triggerType=scheduled, updates schedule lastRunAt', async () => {
        await fc.assert(fc.asyncProperty(arbId, arbId, arbId, async (tenantId, workflowId, scheduleId) => {
            const { handler, putEvents, dynamoPut, dynamoUpdate } = buildPublishedDeps(tenantId, workflowId);
            const event = { workflowId, tenantId, scheduleId };
            await handler(event);
            // Exactly one EventBridge call with correct source and detail-type
            expect(putEvents).toHaveBeenCalledTimes(1);
            const ebCall = putEvents.mock.calls[0]?.[0];
            ...
            const putCall = dynamoPut.mock.calls[0]?.[0];
            ...
            expect(putCall.Item.SK.startsWith('RUN#')).toBe(true);
            ...
            const updateCall = dynamoUpdate.mock.calls[0]?.[0];
...
describe('Feature: event-triggering-routing, Property 14: DomainEvent structure conformance', () => {
    ...
    it('published DomainEvent has all required fields with correct formats', async () => {
        await fc.assert(fc.asyncProperty(arbId, arbId, arbId, async (tenantId, workflowId, scheduleId) => {
            ...
            expect(UUID_V4_RE.test(detail.traceId)).toBe(true);
            ...
            expect(detail.timestamp.endsWith('Z')).toBe(true);
            // payload must include the scheduleId
            expect(detail.payload.scheduleId).toBe(scheduleId);
        }), { numRuns: 100 });
    });
});
