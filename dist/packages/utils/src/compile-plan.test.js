...
describe('compilePlan', () => {
    it('throws CompilationError when a required param is missing', () => {
        expect(() => compilePlan(baseRecipe, {}, activeConnections)).toThrowError(CompilationError);
        try {
            compilePlan(baseRecipe, {}, activeConnections);
        }
        catch (error) {
            expect(error).toBeInstanceOf(CompilationError);
            expect(error.field).toBe('studentId');
        }
    });
    it('throws CompilationError for unknown connectionKey', () => {
        const missingConnectionRecipe = {
            ...baseRecipe,
            steps: [
                {
                    ...baseRecipe.steps[0],
        ...
        expect(() => compilePlan(missingConnectionRecipe, { studentId: 'abc' }, activeConnections)).toThrowError(CompilationError);
    });
    it('successfully compiles with param substitution', () => {
        const result = compilePlan(baseRecipe, { studentId: 's-100' }, activeConnections);
...
//# sourceMappingURL=compile-plan.test.js.map