...
import { applyFieldMappings, BatchSyncThresholdError, buildEnrollmentsUrl, oneRosterConnector, } from './index.js';
describe('applyFieldMappings', () => {
    it('maps fields correctly', () => {
        const result = applyFieldMappings({ sourcedId: 'u1', givenName: 'Ava' }, [
    ...
    it('skips missing source fields and ignores extra record fields', () => {
        const result = applyFieldMappings({ sourcedId: 'u1', familyName: 'Nguyen', role: 'student' }, [
            { sourceField: 'sourcedId', targetField: 'id' },
            { sourceField: 'givenName', targetField: 'firstName' },
        ]);
...
describe('oneRosterConnector threshold behavior', () => {
    it('throws BatchSyncThresholdError when error rate exceeds 20%', async () => {
        ...
        await expect(oneRosterConnector.run(params, {
            tenantId: 'tenant-1',
            runId: 'run-1',
