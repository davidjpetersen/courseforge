...
import { applyFieldMappings, BatchSyncThresholdError, buildEnrollmentsUrl, buildUserIdBatches, ensureErrorThreshold, filterEnrollmentsByOrg, } from './index.js';
...
const safeKeyArbitrary = fc.string({ minLength: 1, maxLength: 12 }).filter((value) => /^[A-Za-z][A-Za-z0-9_]*$/.test(value));
...
describe('OneRoster properties', () => {
    it('property 1: buildEnrollmentsUrl adds the delta filter only when since is provided', () => {
        fc.assert(fc.property(baseUrlArbitrary, fc.option(isoTimestampArbitrary, { nil: undefined }), (baseUrl, since) => {
    ...
    it('property 2: filterEnrollmentsByOrg keeps all and only matching records', () => {
        ...
        }), { maxLength: 100 }), fc.string({ minLength: 1, maxLength: 12 }), (enrollments, targetOrgId) => {
    ...
    it('property 3: buildUserIdBatches deduplicates non-empty ids and chunks them to 50', () => {
        fc.assert(fc.property(fc.array(fc.string(), { maxLength: 200 }), (userIds) => {
    ...
    it('property 4: applyFieldMappings returns exactly the mapped target fields that exist on the input', () => {
        fc.assert(fc.property(fc.dictionary(safeKeyArbitrary, safeValueArbitrary), fc.array(fc.record({
            sourceField: safeKeyArbitrary,
            targetField: safeKeyArbitrary,
        }), { maxLength: 30 }), (record, mappings) => {
    ...
    it('property 5: ensureErrorThreshold matches the 20% failure rule', () => {
        fc.assert(fc.property(fc.integer({ min: 0, max: 200 }), fc.integer({ min: 0, max: 200 }), (total, errorCountCandidate) => {
