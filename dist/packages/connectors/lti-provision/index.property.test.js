...
describe('LTI provision properties', () => {
    it('property 6: createD2LSignature base64url-encodes apiKey, secret, and path', () => {
        fc.assert(fc.property(safeSegment, safeSegment, safeSegment, (apiKey, secret, path) => {
            expect(createD2LSignature(apiKey, secret, path)).toBe(Buffer.from(`${apiKey}:${secret}:${path}`).toString('base64url'));
        }));
    });
    it('property 7: parseLmsError normalizes structured and fallback LMS errors', async () => {
        await fc.assert(fc.asyncProperty(fc.constantFrom('canvas', 'blackboard', 'brightspace'), fc.integer({ min: 400, max: 599 }), fc.boolean(), safeSegment, safeSegment, async (lmsType, status, validJson, code, message) => {
            const response = validJson
                ? new Response(JSON.stringify(lmsType === 'canvas'
                    ? { errors: [{ message }] }
                    : lmsType === 'blackboard'
                        ? { code, message, field: 'name' }
                        : { ErrorCode: code, Message: message }), { status })
