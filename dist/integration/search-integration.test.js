...
describe('Search integration', () => {
    ...
    it('search by keyword in description returns correct results', () => {
        ...
        expect(response.totalCount).toBeGreaterThanOrEqual(1);
        expect(response.results.some((r) => r.name.toLowerCase().includes('notification') ||
            r.description.toLowerCase().includes('notification'))).toBe(true);
    });
    it('search with category filter returns intersection', () => {
        ...
        for (const result of response.results) {
            ...
            const matchesQuery = result.name.toLowerCase().includes('sync') ||
    ...
    it('searchTemplates returns correct subset for keyword + category', () => {
        ...
        for (const t of matched) {
            ...
            const matchesQuery = t.name.toLowerCase().includes('lms') ||
                t.description.toLowerCase().includes('lms');
            expect(matchesQuery).toBe(true);
        }
    });
});
//# sourceMappingURL=search-integration.test.js.map