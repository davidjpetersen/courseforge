...
const arbSimpleStep = (overrides: Partial<RecipeStep> = {}): fc.Arbitrary<RecipeStep> =>
...
const arbSimpleRecipe = fc.array(arbSimpleStep(), { minLength: 1, maxLength: 5 }).map(
  (steps, index) =>
    ({
...
describe('Feature: workflow-management-api, Property 12: Compilation Size Invariant', () => {
  it('output length equals number of steps in the recipe', () => {
    fc.assert(
      fc.property(arbSimpleRecipe, (recipe) => {
...
function hasUnresolvedTemplates(value: unknown): boolean {
  ...
  if (typeof value === 'object' && value !== null) {
    return Object.values(value as Record<string, unknown>).some(hasUnresolvedTemplates);
  }
  return false;
}
    ...
    fc.assert(
      fc.property(
        ...
        (connectionId, secretRef) => {
          ...
          const auth = (result[0]?.params.auth ?? {}) as Record<string, unknown>;
...
describe('Feature: workflow-management-api, Property 15: Compilation Rejects Missing Required Params', () => {
  it('throws CompilationError with the missing param field when a required param is absent', () => {
    fc.assert(
      fc.property(
        arbParamKey,
        fc.array(arbParamKey, { minLength: 0, maxLength: 3 }),
        (missingKey, otherKeys) => {
          ...
          expect((caughtError as CompilationError).field).toBe(missingKey);
        },
      ),
      { numRuns: 100 },
    );
...
describe('Feature: workflow-management-api, Property 16: Compilation Rejects Unresolvable References', () => {
  ...
  it('throws CompilationError when a connectionKey is not in the connections array', () => {
    fc.assert(
      fc.property(arbId, arbId, (connectionKey, otherId) => {
