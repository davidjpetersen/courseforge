...
describe('lti-provision result normalization', () => {
  it('normalizes Canvas validation response', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        new Response(JSON.stringify({ errors: [{ message: 'name is required' }] }), {
          status: 422,
        }),
      ),
    );
  ...
  it('normalizes Blackboard API errors', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        new Response(JSON.stringify({ code: 'BB_400', message: 'invalid placement' }), {
          status: 400,
        }),
      ),
    );
    ...
    ).rejects.toThrow('BB_400: invalid placement');
  ...
  it('normalizes Brightspace API errors', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        new Response(JSON.stringify({ ErrorCode: 'D2L_401', Message: 'unauthorized' }), {
          status: 401,
        }),
      ),
    );
    ...
    ).rejects.toThrow('D2L_401: unauthorized');
