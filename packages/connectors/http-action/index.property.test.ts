...
describe(
  ...
  () => {
    it(
      'resolves all {{secret:*}} and {{context.*}} placeholders before the HTTP request is made',
      async () => {
        await fc.assert(
          fc.asyncProperty(
            httpMethod,
            safeIdentifier, // secretId
            ...
            async (method, secretId, secretValue, contextField, contextValue) => {
              ...
              const httpClient = vi.fn(async (fetchUrl: string, init: RequestInit) => {
              ...
              await executeHttpAction(
                { method, url, body, maxRetries: 1, initialDelayMs: 0 },
                makeContext({ variables: { [contextField]: contextValue } }),
                {
                  secretsClient,
                  httpClient,
    ...
    it(
      'resolves {{context.*}} references in headers before the HTTP request is made',
      async () => {
        await fc.assert(
          fc.asyncProperty(
            httpMethod,
            safeIdentifier, // variable name
            safeContextValue, // variable value
            async (method, varName, varValue) => {
              const url = 'https://example.com/endpoint';
              const capturedHeaders: Record<string, string>[] = [];

              const httpClient = vi.fn(async (_url: string, init: RequestInit) => {
...
describe(
  ...
  () => {
    it(
      'makes exactly maxRetries total attempts when all responses are 5xx, then throws HttpActionError',
      async () => {
        await fc.assert(
          fc.asyncProperty(
            fc.integer({ min: 1, max: 5 }), // maxRetries (= total number of attempts)
            status5xx,
            safeContextValue, // response body from the server
            async (maxRetries, status, responseBody) => {
              const sleep = vi.fn().mockResolvedValue(undefined);
              const httpClient = vi
                .fn()
                .mockResolvedValue(makeFakeResponse(status, responseBody)) as unknown as typeof fetch;
              ...
              const error = thrownError as HttpActionError;
    ...
    it(
      'sleeps with exponential backoff between retry attempts (initialDelayMs * 2^(attempt-1))',
      async () => {
        await fc.assert(
          fc.asyncProperty(
            fc.integer({ min: 2, max: 5 }), // maxRetries ≥ 2 so at least one sleep happens
            fc.integer({ min: 1, max: 100 }), // initialDelayMs
            status5xx,
            async (maxRetries, initialDelayMs, status) => {
              ...
              const httpClient = vi
                .fn()
                .mockResolvedValue(makeFakeResponse(status, 'error')) as unknown as typeof fetch;
    ...
    it(
      'does not retry 4xx responses — throws HttpActionError on the first attempt',
      async () => {
        await fc.assert(
          fc.asyncProperty(
            fc.integer({ min: 2, max: 5 }), // maxRetries ≥ 2 to confirm only 1 call is made
            ...
            async (maxRetries, status, responseBody) => {
              ...
              expect(thrownError).toBeInstanceOf(HttpActionError);
              expect((thrownError as HttpActionError).statusCode).toBe(status);
...
describe(
  ...
  () => {
    it(
      'never includes raw secret values in any structured log entry on a successful request',
      async () => {
        await fc.assert(
          fc.asyncProperty(
            safeIdentifier, // secretId
            ...
            httpMethod,
            async (secretId, secretValue, method) => {
              const logMessages: string[] = [];
              const logger = { log: vi.fn((msg: string) => logMessages.push(msg)) };
              ...
              const httpClient = vi
                .fn()
                .mockResolvedValue(makeFakeResponse(200, 'ok')) as unknown as typeof fetch;
    ...
    it(
      'never includes raw secret values in log entries when retries occur on 5xx',
      async () => {
        await fc.assert(
          fc.asyncProperty(
            safeIdentifier, // secretId
            ...
            async (secretId, secretValue, maxRetries) => {
              ...
              const httpClient = vi
                .fn()
                .mockResolvedValue(makeFakeResponse(503, 'service unavailable')) as unknown as typeof fetch;
