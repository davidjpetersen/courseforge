...
- [ ] 1. Add dependencies and extend data model
  - [ ] 1.1 Add `@aws-sdk/client-secrets-manager` and `ajv` to `package.json` dependencies
    - Run `npm install @aws-sdk/client-secrets-manager ajv`
    - _Requirements: 1.2, 7.2_

  - [ ] 1.2 Add connection and audit types to `src/models/types.ts`
  ...
  - [ ] 1.3 Extend `src/models/schema.ts` with connection and audit key builders
...
- [ ] 2. Implement connector registry
  - [ ] 2.1 Create `src/api/connections/registry.ts` with connector definitions
  ...
  - [ ]* 2.2 Write property test: Registry definition completeness (Property 10)
    - **Property 10: Connector registry definition completeness**
    - **Validates: Requirements 7.2**
    - File: `src/api/connections/logic.property.test.ts`

  - [ ]* 2.3 Write property test: OAuth connector stub behavior (Property 11)
    - **Property 11: OAuth connector stub behavior**
    - **Validates: Requirements 7.6**
    - File: `src/api/connections/logic.property.test.ts`

- [ ] 3. Implement pure logic functions
  - [ ] 3.1 Create `src/api/connections/logic.ts` with `validateCredentials`
  ...
  - [ ] 3.2 Add `mapConnectionToListItem` to `src/api/connections/logic.ts`
    - Map `ConnectionRecord` to `ConnectionListItem`, stripping `secretRef` and all credential fields
    - _Requirements: 1.6, 2.5, 3.2, 3.3_

  - [ ] 3.3 Add `mapTestResultToStatus`, `filterDependentWorkflows`, `hasPublishedDependents`, and `buildAuditEntry` to `src/api/connections/logic.ts`
  ...
  - [ ] 3.4 Add `buildNewConnectionRecord` to `src/api/connections/logic.ts`
    - Constructs a `ConnectionRecord` with status `'pending'`, generated UUID, and `createdAt === updatedAt`
    - _Requirements: 1.3, 1.4_

  - [ ]* 3.5 Write property test: Credential validation correctness (Property 1)
    - **Property 1: Credential validation correctness**
    - **Validates: Requirements 1.1, 1.5**
    - File: `src/api/connections/logic.property.test.ts`

  - [ ]* 3.6 Write property test: Secret naming convention (Property 2)
    - **Property 2: Secret naming convention**
    - **Validates: Requirements 1.2**
    - File: `src/api/connections/logic.property.test.ts`

  - [ ]* 3.7 Write property test: New connection initial state (Property 3)
    - **Property 3: New connection initial state**
    - **Validates: Requirements 1.3**
    - File: `src/api/connections/logic.property.test.ts`

  - [ ]* 3.8 Write property test: Response credential exclusion (Property 4)
    - **Property 4: Response credential exclusion**
    - **Validates: Requirements 1.6, 2.5, 3.2, 3.3**
    - File: `src/api/connections/logic.property.test.ts`

  - [ ]* 3.9 Write property test: Test result to status mapping (Property 5)
    - **Property 5: Test result to status mapping**
    - **Validates: Requirements 2.2, 2.3, 2.4**
    - File: `src/api/connections/logic.property.test.ts`

  - [ ]* 3.10 Write property test: Dependency filtering correctness (Property 6)
    - **Property 6: Dependency filtering correctness**
    - **Validates: Requirements 4.1, 4.2**
    - File: `src/api/connections/logic.property.test.ts`

  - [ ]* 3.11 Write property test: Audit entry construction (Property 7)
    - **Property 7: Audit entry construction**
    - **Validates: Requirements 5.4, 6.5**
    - File: `src/api/connections/logic.property.test.ts`

  - [ ]* 3.12 Write property test: Published dependency guard (Property 8)
    - **Property 8: Published dependency guard**
    - **Validates: Requirements 6.1, 6.2**
    - File: `src/api/connections/logic.property.test.ts`

  - [ ]* 3.13 Write property test: Soft-delete state transition (Property 9)
...
- [ ] 5. Implement repository and secrets service interfaces
  - [ ] 5.1 Create `ConnectionRepository` and `AuditRepository` interfaces in `src/api/connections/handlers.ts`
...
- [ ] 6. Implement API route handlers
  - [ ] 6.1 Implement `createConnectionHandler` in `src/api/connections/handlers.ts`
  ...
  - [ ] 6.2 Implement `testConnectionHandler` in `src/api/connections/handlers.ts`
    - Factory function accepting `(repo, secrets, registry)`
  ...
  - [ ] 6.3 Implement `listConnectionsHandler` in `src/api/connections/handlers.ts`
    - Factory function accepting `(repo)`
    - Lists connections by tenant, maps each through `mapConnectionToListItem`
    - _Requirements: 3.1, 3.2, 3.3_

  - [ ] 6.4 Implement `getDependenciesHandler` in `src/api/connections/handlers.ts`
  ...
  - [ ] 6.5 Implement `rotateConnectionHandler` in `src/api/connections/handlers.ts`
  ...
  - [ ] 6.6 Implement `deleteConnectionHandler` in `src/api/connections/handlers.ts`
  ...
  - [ ]* 6.7 Write unit tests for all handlers in `src/api/connections/handlers.test.ts`
...
- [ ] 7. Create barrel export and wire modules
  - [ ] 7.1 Create `src/api/connections/index.ts` barrel export
  ...
  - [ ]* 7.2 Write unit tests for pure logic functions in `src/api/connections/logic.test.ts`
