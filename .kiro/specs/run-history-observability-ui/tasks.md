...
- [ ] 1. Add shared type definitions and schema key builders
  - [ ] 1.1 Add `Run`, `RunStep`, and `Notification` interfaces to `packages/types/src/events.ts` and export them from `packages/types/src/index.ts`
  ...
  - [ ] 1.2 Add new key prefixes, key builders, and GSI constants to `src/models/schema.ts`
  ...
  - [ ]* 1.3 Write unit tests for new key builders in `src/models/schema.test.ts`
    - Test `runPK`, `stepSK` (zero-padded index), `userPK`, `notificationSK`
    - Test GSI constant values
    - _Requirements: 1.2, 1.3, 2.1, 4.1_

- [ ] 2. Implement masking utility
  - [ ] 2.1 Create `src/lib/mask-sensitive.ts` with `maskSensitiveFields` function
  ...
  - [ ]* 2.2 Write unit tests for masking utility in `src/lib/mask-sensitive.test.ts`
  ...
  - [ ]* 2.3 Write property test for masking idempotence in `src/lib/mask-sensitive.property.test.ts`
...
- [ ] 3. Implement query parameter validation
  - [ ] 3.1 Create `src/api/runs/validation.ts` with `validateRunsQueryParams`, `isValidISODate`, `clampLimit`, `encodeCursor`, `decodeCursor`
  ...
  - [ ]* 3.2 Write unit tests for validation in `src/api/runs/validation.test.ts`
  ...
  - [ ]* 3.3 Write property tests for validation in `src/api/runs/validation.property.test.ts`
...
- [ ] 5. Implement Runs API handler
  - [ ] 5.1 Create `src/api/runs/handler.ts` with `RunRepository` interface and `createRunsHandler` factory
  ...
  - [ ]* 5.2 Write unit tests for Runs API handler in `src/api/runs/handler.test.ts`
...
- [ ] 6. Implement Run Detail API handler
  - [ ] 6.1 Create `src/api/runs/detail-handler.ts` with `S3Client` interface and `createRunDetailHandler` factory
  ...
  - [ ]* 6.2 Write unit tests for Run Detail handler in `src/api/runs/detail-handler.test.ts`
...
- [ ] 7. Implement Notifications API handlers
  - [ ] 7.1 Create `src/api/notifications/handler.ts` with `NotificationRepository` interface, `createNotificationsHandler`, and `createNotificationReadHandler`
  ...
  - [ ]* 7.2 Write unit tests for Notifications handlers in `src/api/notifications/handler.test.ts`
...
- [ ] 9. Implement Status Badge utility
  - [ ] 9.1 Create `src/ui/status-badge.ts` with `STATUS_BADGE_MAP` and `getStatusBadge` function
  ...
  - [ ]* 9.2 Write unit tests for status badge in `src/ui/status-badge.test.ts`
    - Test each RunStatus maps to correct label, colorClass, and animate flag
    - Test RUNNING has `animate: true`
    - _Requirements: 5.2_

- [ ] 10. Implement Run List page logic
  - [ ] 10.1 Create `src/ui/run-list.ts` with state management and pure functions
  ...
  - [ ]* 10.2 Write unit tests for run list logic in `src/ui/run-list.test.ts`
  ...
  - [ ]* 10.3 Write property test for sortFailedFirst in `src/ui/run-list.property.test.ts`
    - **Property 4: sortFailedFirst preserves all elements — output is a permutation of input (same length, same elements)**
    - **Validates: Requirement 5.5**

- [ ] 11. Implement Run Detail page logic
  - [ ] 11.1 Create `src/ui/run-detail.ts` with state management and pure functions
  ...
  - [ ]* 11.2 Write unit tests for run detail logic in `src/ui/run-detail.test.ts`
...
- [ ] 12. Implement NotificationBell logic
  - [ ] 12.1 Create `src/ui/notification-bell.ts` with state management and pure functions
  ...
  - [ ]* 12.2 Write unit tests for notification bell logic in `src/ui/notification-bell.test.ts`
  ...
  - [ ]* 12.3 Write property test for formatRelativeTime in `src/ui/notification-bell.property.test.ts`
