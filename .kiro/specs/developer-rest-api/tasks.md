...
- [x] 1. Add schema key builders and shared types
  - [x] 1.1 Add API key and rate limit key builders to `src/models/schema.ts`
  ...
  - [x] 1.2 Add API key types to `packages/types/src/`
    - Create `packages/types/src/api-keys.ts` with `ApiKeyRecord`, `ApiKeyScope`, and `EndpointGroup` types
    - Export from `packages/types/src/index.ts`
    - _Requirements: 1.3, 5.2_

- [x] 2. Implement API Key Manager handler and repository
  - [x] 2.1 Create `src/api/developer-keys/repository.ts`
  ...
  - [x] 2.2 Create `src/api/developer-keys/handler.ts`
  ...
  - [x] 2.3 Write property tests for API Key Manager (`src/api/developer-keys/handler.property.test.ts`)
  ...
  - [x] 2.4 Write unit tests for API Key Manager (`src/api/developer-keys/handler.test.ts`)
...
- [x] 3. Implement API Key Auth Middleware
  - [x] 3.1 Create `src/api/middleware/api-key-auth.ts`
  ...
  - [x] 3.2 Write property tests for auth middleware (`src/api/middleware/api-key-auth.property.test.ts`)
    - **Property 4: Auth middleware correctness** — valid token returns AuthContext, invalid/revoked returns 401
    - **Validates: Requirements 4.1, 4.2, 4.3, 4.5**
  - [x] 3.3 Write unit tests for auth middleware (`src/api/middleware/api-key-auth.test.ts`)
    - Test valid key, disabled key, missing header, malformed token, lastUsedAt async update
    - _Requirements: 4.1–4.5_

- [x] 4. Implement Rate Limiter Middleware
  - [x] 4.1 Create `src/api/middleware/rate-limiter.ts`
  ...
  - [x] 4.2 Write property tests for rate limiter (`src/api/middleware/rate-limiter.property.test.ts`)
  ...
  - [x] 4.3 Write unit tests for rate limiter (`src/api/middleware/rate-limiter.test.ts`)
...
- [x] 5. Implement Scope Enforcer Middleware
  - [x] 5.1 Create `src/api/middleware/scope-enforcer.ts`
  ...
  - [x] 5.2 Write property tests for scope enforcer (`src/api/middleware/scope-enforcer.property.test.ts`)
    - **Property 10: Scope enforcement** — read scope + mutation = 403, GET always allowed
    - **Validates: Requirements 14.1, 14.2, 14.3**
  - [x] 5.3 Write unit tests for scope enforcer (`src/api/middleware/scope-enforcer.test.ts`)
    - Test read scope with GET, POST, DELETE; write scope with all methods; events endpoint
    - _Requirements: 14.1–14.3_

- [x] 6. Checkpoint - Ensure all middleware tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. Implement V1 Route Handlers
  - [x] 7.1 Create `src/api/v1/recipes.ts`
  ...
  - [x] 7.2 Write property and unit tests for recipes handler (`src/api/v1/recipes.property.test.ts`, `src/api/v1/recipes.test.ts`)
    - **Property 16: Recipe listing returns complete objects**
    - **Validates: Requirements 6.1, 6.2**
  - [x] 7.3 Create `src/api/v1/workflows.ts`
  ...
  - [x] 7.4 Write property and unit tests for workflows handler (`src/api/v1/workflows.property.test.ts`, `src/api/v1/workflows.test.ts`)
  ...
  - [x] 7.5 Create `src/api/v1/events.ts`
  ...
  - [x] 7.6 Write property and unit tests for events handler (`src/api/v1/events.property.test.ts`, `src/api/v1/events.test.ts`)
  ...
  - [x] 7.7 Create `src/api/v1/runs.ts`
  ...
  - [x] 7.8 Write property and unit tests for runs handler (`src/api/v1/runs.property.test.ts`, `src/api/v1/runs.test.ts`)
...
- [x] 9. Implement OpenAPI Spec Generator
  - [x] 9.1 Create `src/api/v1/openapi.ts`
  ...
  - [x] 9.2 Write unit tests for OpenAPI spec (`src/api/v1/openapi.test.ts`)
    - Validate spec structure, required fields, paths, schemas, security schemes
    - _Requirements: 15.1–15.6_

- [x] 10. Wire Next.js API routes
  - [x] 10.1 Create `app/api/developer/keys/route.ts`
  ...
  - [x] 10.2 Create `app/api/developer/keys/[keyId]/route.ts`
    - Wire DELETE (revoke) to `createApiKeyHandler.revoke`
    - _Requirements: 3.1_
  - [x] 10.3 Create `app/api/v1/recipes/route.ts`
    - Wire GET through auth → rate limiter → scope enforcer → recipe handler
    - _Requirements: 6.1_
  - [x] 10.4 Create `app/api/v1/workflows/route.ts`
    - Wire POST (create) and GET (list) through middleware chain
    - _Requirements: 7.1, 8.1_
  - [x] 10.5 Create `app/api/v1/workflows/[workflowId]/route.ts`
    - Wire GET (detail) through middleware chain
    - _Requirements: 9.1_
  - [x] 10.6 Create `app/api/v1/workflows/[workflowId]/publish/route.ts`
    - Wire POST (publish) through middleware chain
    - _Requirements: 10.1_
  - [x] 10.7 Create `app/api/v1/events/route.ts`
    - Wire POST through auth → rate limiter (events group) → scope enforcer → event handler
    - _Requirements: 11.1_
  - [x] 10.8 Create `app/api/v1/runs/route.ts`
    - Wire GET (list) through middleware chain
    - _Requirements: 12.1_
  - [x] 10.9 Create `app/api/v1/runs/[runId]/route.ts`
    - Wire GET (detail) through middleware chain
    - _Requirements: 13.1_
  - [x] 10.10 Create `app/api/v1/openapi.json/route.ts`
    - Wire GET to `generateOpenApiSpec()` with no authentication required
    - Return JSON with `Content-Type: application/json`
    - _Requirements: 15.6_

- [x] 11. Implement Developer Portal and Swagger UI pages
  - [x] 11.1 Create `app/(dashboard)/developer/docs/page.tsx`
    - Render Swagger UI loaded from `unpkg.com/swagger-ui-dist` CDN
    - Point at `/api/v1/openapi.json` spec URL
    - _Requirements: 16.1, 16.2_
  - [x] 11.2 Create `app/(dashboard)/developer/page.tsx`
