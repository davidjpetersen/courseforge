...
- [ ] 1. Extend domain types and data model
  - [ ] 1.1 Add trigger-related types to `packages/types/src/events.ts`
  ...
  - [ ] 1.2 Add key builders and schema constants to `src/models/schema.ts`
...
- [ ] 2. Implement WebhookIngressFn Lambda handler
  - [ ] 2.1 Create `functions/webhook-ingress/handler.ts`
  ...
  - [ ]* 2.2 Write unit tests for WebhookIngressFn (`functions/webhook-ingress/handler.test.ts`)
  ...
  - [ ]* 2.3 Write property test: Webhook authentication correctness (`functions/webhook-ingress/handler.property.test.ts`)
    - **Property 1: Webhook authentication correctness**
    - **Validates: Requirements 4.2, 4.3**

  - [ ]* 2.4 Write property test: Non-PUBLISHED workflow rejection (`functions/webhook-ingress/handler.property.test.ts`)
    - **Property 2: Non-PUBLISHED workflow rejection (webhook)**
    - **Validates: Requirements 5.1, 5.2**

  - [ ]* 2.5 Write property test: Successful webhook processing produces complete outputs (`functions/webhook-ingress/handler.property.test.ts`)
    - **Property 3: Successful webhook processing produces complete outputs**
    - **Validates: Requirements 5.5, 5.6, 5.7, 5.8**

- [ ] 3. Implement ScheduledTriggerFn Lambda handler
  - [ ] 3.1 Create `functions/scheduled-trigger/handler.ts`
  ...
  - [ ]* 3.2 Write unit tests for ScheduledTriggerFn (`functions/scheduled-trigger/handler.test.ts`)
  ...
  - [ ]* 3.3 Write property test: Non-PUBLISHED workflow silent return (`functions/scheduled-trigger/handler.property.test.ts`)
    - **Property 4: Non-PUBLISHED workflow silent return (scheduled)**
    - **Validates: Requirements 6.1, 6.2**

  - [ ]* 3.4 Write property test: Successful scheduled trigger produces complete outputs (`functions/scheduled-trigger/handler.property.test.ts`)
    - **Property 5: Successful scheduled trigger produces complete outputs**
    - **Validates: Requirements 6.3, 6.4, 6.5, 6.6, 6.7**

- [ ] 4. Checkpoint — Verify trigger handlers
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. Implement HTTP Action Step Connector
  - [ ] 5.1 Create `packages/connectors/http-action/index.ts`
  ...
  - [ ]* 5.2 Write unit tests for HTTP Action Connector (`packages/connectors/http-action/index.test.ts`)
  ...
  - [ ]* 5.3 Write property test: Template reference resolution (`packages/connectors/http-action/index.property.test.ts`)
    - **Property 6: Template reference resolution**
    - **Validates: Requirements 7.2, 7.3**

  - [ ]* 5.4 Write property test: Retry with exponential backoff and error propagation (`packages/connectors/http-action/index.property.test.ts`)
    - **Property 7: Retry with exponential backoff and error propagation**
    - **Validates: Requirements 7.4, 7.5**

  - [ ]* 5.5 Write property test: Secret exclusion from logs (`packages/connectors/http-action/index.property.test.ts`)
    - **Property 8: Secret exclusion from logs**
    - **Validates: Requirements 7.6, 7.7**

- [ ] 6. Implement Trigger Management API — Webhook Secret
  - [ ] 6.1 Create `src/api/triggers/webhook-secret.ts`
  ...
  - [ ]* 6.2 Write unit tests for webhook secret handler (`src/api/triggers/webhook-secret.test.ts`)
    - Test creation returns 32-byte hex token
    - Test stored value is SHA-256 hash, not raw token
    - _Requirements: 8.1, 8.2, 8.3_

  - [ ]* 6.3 Write property test: Webhook secret hash round-trip (`src/api/triggers/webhook-secret.property.test.ts`)
    - **Property 9: Webhook secret hash round-trip**
    - **Validates: Requirements 8.1, 8.2, 8.3**

- [ ] 7. Implement Trigger Management API — Schedule CRUD
  - [ ] 7.1 Create `src/api/triggers/schedule.ts`
  ...
  - [ ]* 7.2 Write unit tests for schedule handlers (`src/api/triggers/schedule.test.ts`)
  ...
  - [ ]* 7.3 Write property test: Cron expression syntax validation (`src/api/triggers/schedule.property.test.ts`)
    - **Property 10: Cron expression syntax validation**
    - **Validates: Requirements 9.1**

  - [ ]* 7.4 Write property test: Minimum schedule interval enforcement (`src/api/triggers/schedule.property.test.ts`)
    - **Property 11: Minimum schedule interval enforcement**
    - **Validates: Requirements 9.2**

  - [ ]* 7.5 Write property test: Schedule creation produces complete outputs (`src/api/triggers/schedule.property.test.ts`)
    - **Property 12: Schedule creation produces complete outputs**
    - **Validates: Requirements 9.3, 9.4, 9.5**

  - [ ]* 7.6 Write property test: Schedule deletion soft-deletes record (`src/api/triggers/schedule.property.test.ts`)
    - **Property 13: Schedule deletion soft-deletes record**
    - **Validates: Requirements 10.1, 10.2**

- [ ] 8. Checkpoint — Verify all application logic
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Implement DomainEvent conformance property tests
  - [ ]* 9.1 Write property test: DomainEvent structure conformance (`functions/webhook-ingress/handler.property.test.ts`)
    - **Property 14: DomainEvent structure conformance**
    - Test both WebhookIngressFn and ScheduledTriggerFn event outputs
    - **Validates: Requirements 11.1, 11.2**

  - [ ]* 9.2 Write property test: DomainEvent serialization round-trip (`functions/webhook-ingress/handler.property.test.ts`)
    - **Property 15: DomainEvent serialization round-trip**
    - **Validates: Requirements 11.3**

- [ ] 10. Implement TriggerStack CDK infrastructure
  - [ ] 10.1 Create `infra/lib/trigger-stack.ts`
  ...
  - [ ]* 10.2 Write CDK assertion tests (`infra/test/trigger-stack.test.ts`)
...
- [ ] 11. Wire TriggerStack into CDK app entry point
  - [ ] 11.1 Update `infra/bin/` CDK app to instantiate `TriggerStack` with references to FoundationStack outputs
